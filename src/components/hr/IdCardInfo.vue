...
;(async () => {
  ...
  politys.value = await fetchDict('browser.hr_polity')
})()
...
async function onCountryConfirm({ selectedOptions }: any) {
  ...
  if (selectedOptions) {
    ...
    // 按国家加载证件类型字典
    idTypes.value = await fetchDict('browser.hr_id_type', { country: form.country })
    const first = idTypes.value[0]
    form.idTypeText = first?.text || ''
    form.idType = first?.value || ''
    const isChina = String(form.country) === '1'
    form.showHukou = isChina
    form.showIdNumberStartDate = isChina
    form.showIdNumberEndDate = isChina
    form.idNumberStartDateRequired = isChina
    form.idNumberEndDateRequired = isChina
  }
}
